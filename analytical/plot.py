...
def mk_plot( area_constraints=[ 0.115, 0.3 ] ):
  msg_sizes = [ 64, 512 ]
  ...
  for r, msg_size in enumerate( msg_sizes ):
    ...
    for c, A_constraint in enumerate( area_constraints ):
      ...
      for data in data_sets:
        ...
        lat = [ x.h_lat + x.s_lat for x in data if x.area <= A_constraint and x.chnl <= msg_size ]
        bw  = [ x.bw for x in data if x.area <= A_constraint and x.chnl <= msg_size ]
        ...
        if len( lat ) > 0:
          ...
          continue
        axes[r][c].plot( optimal_bw, optimal_lat, style, color=color, label=data[0].name, markersize=4, linewidth=0 )
        axes[r][c].spines['top'  ].set_visible( False )
        axes[r][c].spines['right'].set_visible( False )
        axes[r][c].set_ylim( top=limit[r][c]*1.1, bottom=0 )
        # axes[r][c].grid()
  ...
  plt.savefig( f'lat-bw.pdf', bbox_inches='tight' )
