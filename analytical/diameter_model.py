...
def mesh_cxs1_lat( ncols, nrows, ncores_per_tile, ser_lat, channel_lat ):
  ...
  hops = [ hr + hc*channel_lat for hr, hc in zip( channel_hops, router_hops ) ]
  return max( hops ), h_lat
...
def torus_c1s1_lat( ncols, nrows, ser_lat, channel_lat ):
  ...
  return max( [ hr+hc for hr, hc in zip(router_hops, channel_hops) ] ), avg_channel_hop + avg_router_hop
...
def mesh_cxs2_lat( ncols, nrows, ncores_per_tile, exp_channel_lat, ser_lat, channel_lat ):
  ...
  return max( all_lat ) ,sum( all_lat ) / len( all_lat )
...
if __name__ == '__main__':
  ...
  ts   = 1
  chnl = 1
  print( mesh_cxs1_lat( 16, 16, 1, ts, chnl ) )
  print( mesh_cxs1_lat( 8, 8, 4, ts, chnl ) )
  print( mesh_cxs1_lat( 4, 8, 8, ts, chnl ) )

  print( mesh_cxs2_lat( 16, 16, 1, 1, ts, chnl ) )
  print( mesh_cxs2_lat( 8,  8,  4, 1, ts, chnl ) )
  print( mesh_cxs2_lat( 4,  8,  8, 1, ts, chnl ) )

  print( mesh_cxs3_lat( 16, 16, 1, 1, ts, chnl ) )
  print( mesh_cxs3_lat( 8, 8, 4, 1, ts, chnl ) )
  print( mesh_cxs3_lat( 4, 8, 8, 2, ts, chnl ) )

  print( torus_c1s1_lat( 16, 16, ts, chnl ) )
  print( torus_c1s1_lat( 8, 8, ts, chnl ) )
  print( torus_c1s1_lat( 4, 8, ts, chnl ) )