...
#        network.connect( routers[i].send[EAST],  routers[routers[i].router_id+1].recv[WEST]  )
      network.connect( routers[i].send[NORTH], routers[(routers[i].router_id-rows+num_routers)%num_routers].recv[SOUTH] )
      network.connect( routers[i].send[SOUTH], routers[(routers[i].router_id+rows+num_routers)%num_routers].recv[NORTH] )
      network.connect( routers[i].send[WEST],  routers[(routers[i].router_id-1+num_routers)%num_routers].recv[EAST]  )
      network.connect( routers[i].send[EAST],  routers[(routers[i].router_id+1+num_routers)%num_routers].recv[WEST]  )
