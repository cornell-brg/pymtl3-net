...
from torusnet.TorusRouterCL   import TorusRouterCL
...
class TestHarness( Component ):
  ...
  def construct( s, 
    ...
    print "=" * 74
    MeshPos = mk_mesh_pos( wid, ht )
    s.dut = TorusRouterCL( MsgType, MeshPos )
  ...
  def line_trace( s ):
    return "{} - {} - {}".format( 
      "|".join( [ s.srcs[i].line_trace() for i in range(5) ] ),
      s.dut.line_trace(),
...
def test_normal_simple():
  ...
  wid = 2
  ht  = 2
  pkt0 = mk_pkt( 1 % wid, 1 / wid, 2 % wid, 2 / wid, 0x01, 0xface )
#  pkt0 = mk_base_pkt( 1, 2, 0x01, 0xface )
  ...
  th = TestHarness( Packet, 2, 2, 1, src_packets, sink_packets, 0, 0, 0, 0 )
  run_sim( th )
