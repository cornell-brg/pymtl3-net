...
Add a single cycle straight pass and register the routing output.
...
from .PitonRoutingLogic import PitonRoutingLogic
...
class PitonRouteUnitOpt( Component ):
  ...
  def construct( s, PositionType, RoutingLogicType=PitonRoutingLogic,
    ...
    s.STATE_HEADER       = 0
    s.STATE_NORMAL_BODY  = 1
    s.STATE_EXPRESS_BODY = 2
    ...
    def up_state_next():
      ...
      if s.state == s.STATE_HEADER:
        ...
        if s.any_give_en & ( s.header.plen > 0 ):
          s.state_next @= s.STATE_NORMAL_BODY

      else: # STATE_NORMAL_BODY
    ...
    def up_counter_load():
      s.counter.load @= 0
      if s.state == s.STATE_HEADER:
        s.counter.load @= ( s.state_next == s.STATE_NORMAL_BODY )
    ...
    def up_out_dir():
      ...
      if ( s.state == s.STATE_HEADER ) & s.get.rdy:
        s.out_dir @= s.routing_logic.out_dir
    ...
    def up_give_rdy_hold():
      for i in range( s.num_outports ):
        ...
        s.hold[i]     @= ( i == s.out_dir ) & ( s.state == s.STATE_NORMAL_BODY )
  ...
  def line_trace( s ):
    ...
    state = 'H' if s.state == s.STATE_HEADER else \
            'B' if s.state == s.STATE_NORMAL_BODY else   \
            '!'
