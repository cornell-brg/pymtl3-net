...
class PitonRouteUnitOpt( Component ):
  ...
  def construct( s, PositionType, plen_field_name='plen' ):
    ...
    s.in_  = InValRdyIfc( s.PhitType )
    s.pos  = InPort( PositionType ) # TODO: figure out a way to encode position

    s.out  = [ OutValRdyIfc( PitonInternalFlit ) for _ in range( s.num_outports ) ]
    ...
    connect_bitstruct( s.in_.msg, s.header )

    @update
    def up_out_msg():
      for i in range( 5 ):
        s.out[i].msg.flit @= s.in_.msg
        s.out[i].msg.is_head @= ( s.state == s.STATE_HEADER )

        if s.state == s.STATE_HEADER:
          s.out[i].msg.is_tail @= ( s.header.plen == 0 )
        else:
          s.out[i].msg.is_tail @= ( s.counter.count == 1 )

    # s.in_.rdy //= s.any_give_en

    @update
    def up_in_rdy():
      s.in_.rdy @= s.out[ s.out_dir ].rdy
  ...
  def line_trace( s ):
    give_trace = '|'.join([ f'{ifc}' for ifc in s.out ])
    ...
    dir   = 'n' if s.out_dir == NORTH else \
            ...
            '?'
    return f'{s.in_}({pos}[{state}{count}]{dir}{hold}){give_trace}'
