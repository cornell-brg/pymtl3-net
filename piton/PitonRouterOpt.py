...
from .PitonSwitchUnitOpt import PitonSwitchUnitOpt
...
class PitonRouterOpt( Component ):

  def construct( s,
    PositionType,
    InputUnitType   = InputUnitValRdy,
    RouteUnitType   = PitonRouteUnitOpt,
    SwitchUnitType  = PitonSwitchUnitOpt,
    OutputUnitType  = OutputUnitValRdy,
    straight_srcs   = straight_srcs,
  ):
    ...
    s.switch_units = [ SwitchUnitType( s.num_inports, speculative_input=straight_srcs[i] )
  ...
  def line_trace( s ):
    ...
    # return f'{s.switch_units[2].line_trace()}'
    # return f'{s.route_units[0].line_trace()}'
